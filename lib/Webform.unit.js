...
describe('Webform tests', function () {
  ...
  describe('Validate onBlur', function () {
    it('Should keep component valid onChange when validate trigger is onBlur', function (done) {
      formElement.innerHTML = '';
      var form = new _Webform.default(formElement, {
        language: 'en',
        template: 'bootstrap3'
      ...
      form.setForm(_formtest.validationOnBlur).then(function () {
        ...
        _harness.default.clickElement(form, fieldInput);

        var inputEvent = new Event('input');
        fieldInput.value = '12';
        fieldInput.dispatchEvent(inputEvent);
        setTimeout(function () {
          _powerAssert.default.equal(field.errors.length, 0);

          var blurEvent = new Event('blur');
          fieldInput.dispatchEvent(blurEvent);
          setTimeout(function () {
            _powerAssert.default.equal(field.errors.length, 1);
